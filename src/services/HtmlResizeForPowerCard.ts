...
export class HtmlResizeForPowerCard {
  public static changeStylesDependingOnWidth(
    ...
    // icons
    teslaCardElement.querySelectorAll('.acc_container').forEach((_currentValue, currentIndex, iconContainerItem) => {
    ...
    changeSelectorStyle(
      ...
      'd',
      'M' + (half - pxRate) + ',0 C' + (half - pxRate) + ',' + half + ' ' + (half - pxRate) + ',' + half + ' ' + half * 2 + ',' + half
    );
    changeSelectorStyle(
      '#grid_feed_in_entity_line',
      'd',
      'M' + (half - pxRate) + ',0 C' + (half - pxRate) + ',' + half + ' ' + (half - pxRate) + ',' + half + ' 0,' + half
    );
    changeSelectorStyle(
    ...
    changeSelectorStyle(
      ...
      'd',
      'M' + (half - pxRate) + ',0 C' + (half - pxRate) + ',0 ' + (half - pxRate) + ',' + half * 2 + ' ' + (half - pxRate) + ',' + half * 2
    );
    ...
    [1, 2].forEach(value => {
      ...
      changeSelectorStyle('.acc_appliance' + value + '_line', 'right', 9.5 * pxRate + 10 + 'px');
      changeSelectorStyle('.acc_appliance' + value + '_line', 'width', '10px');
      changeSelectorStyle('.acc_appliance' + value + '_line', 'height', 12 * pxRate - (value - 1) * 5 + 'px');
      changeSelectorStyle('.acc_appliance' + value + '_line svg', 'width', '10px');
      ...
      const bottomElement = <HTMLElement>teslaCardElement.querySelector('.battery_entity');
      if (bottomElement === null && value === 2 && selectorElement !== null) {
        changeSelectorStyle('.acc_center_container', 'margin-bottom', 19 * pxRate + 'px');
      }
    });
    const gridElement = <HTMLElement>teslaCardElement.querySelector('.grid_entity');
    if (gridElement === null) {
      ...
      selectorElement = <HTMLElement>teslaCardElement.querySelector('.power_lines svg');
      if (selectorElement !== null) selectorElement.setAttribute('viewBox', 12 * pxRate + ' 0 ' + 42 * pxRate + ' ' + 42 * pxRate);
    }
